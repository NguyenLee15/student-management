...
export default function CourseRegistrationView() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-700 via-indigo-700 to-purple-800 rounded-2xl p-6 text-white shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="space-y-2">
          ...
          <h1 className="text-2xl font-black">
            {activePeriod?.name || 'Đợt 1 - Đăng Ký Tín Chỉ Chính Thức'}
          </h1>
          <div className="flex flex-wrap items-center gap-4 text-xs text-blue-100 pt-1">
            <span className="flex items-center gap-1.5">
              <Calendar className="w-4 h-4 text-blue-300" />
              Bắt đầu: {activePeriod?.startTime ? new Date(activePeriod.startTime).toLocaleDateString('vi-VN') : '24/08/2026'}
            </span>
            <span className="flex items-center gap-1.5">
              <Clock className="w-4 h-4 text-blue-300" />
              Hạn chót: {activePeriod?.endTime ? new Date(activePeriod.endTime).toLocaleDateString('vi-VN') : '07/09/2026'}
            </span>
